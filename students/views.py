...
def leave_name_field(request):
    ...
    QuerySet = students.objects.filter( s_email = request.session['smail'] )[0]
    print(QuerySet)
    return render (request,'student_new_leave.html',{'studentname':QuerySet})
...
def attendence_table(request):
    ab = request.GET.get('id')
    #attendenceSet = attendence.objects.all()
    attendenceSet = attendence.objects.filter(s_name = ab)
    print(attendenceSet)
...
#student leave table view
def leave_table_view(request):
    ab = request.GET.get('lname')
    leaveSet = studentleave.objects.filter(s_name = ab)
    return render(request,'student_attendence.html',{'leavetable':leaveSet})